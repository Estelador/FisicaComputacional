...
int main(){
    ...
    int nciclos,Nt;
    double k0, lambda, s;
    double norma,A_m, pos, pos2, dx, dp, dEc, dEt,prob, Ep;
    ...
    sch=fopen("sch.txt","w");
    pot=fopen("pot.txt","w");
    posicion=fopen("posicion_s.txt","w");
    momento=fopen("momento_s.txt","w");
    etotal=fopen("etotal_s.txt","w");
    ecin=fopen("ecin_s.txt","w");
    epot=fopen("epot_s.txt","w");
    ...
    for (i=1;i<=Nt;i++){
        ...
        //Medimos en t=nd
        if(i==500){
            //Colapsamos la funcion de onda, por tanto inicializamos la probabilidad a 0
            prob=0.0;
            for(j=(4*N/5);j<=N;j++){
                prob=prob+Cabs(Conjg(phi[j]))*Cabs(phi[j]);
                phi[j].r=phi[j].r=0.0;
            }
            //Redistribuimos la probabildad en el espacio
            for(j=0;j<=(4*N/5);j++){
                phi[j].r=phi[j].r/sqrt(1-prob);
                phi[j].i=phi[j].i/sqrt(1-prob);
            }
