...
int main(){
    ...
    //Variables
    int i,j,k;
    int nciclos,Nd,t,Ntransmision;
    double k0, lambda,s,r,var2,error;
    double norma,n1,n2,Pi,Pd,aleat,aleat1,T;
    double V[N+1],A_p[N+1],A_p_aux[N+1];
    double transmision[1000];
    ...
    //Inicializamos el factor de transmision y la varianza al cuadrado
    Ntransmision=0;
    var2=0.0;
    ...
    for (k=1;k<=Nexp;k++){
        ...
        //Escribimos  en un fichero los coeficientes de transmision obtenidos en las k simulaciones
        transmision[k]=(1.0*Ntransmision)/(k*1.0);
        fprintf(resultados, "%i \t %lf \n",k,transmision[k]);
    ...
    //Hallamos la varianza al cuadrado
    var2=var2+pow((transmision[k]-T),2);
    var2=var2/Nexp;
    
    //Lo hacemos con error de 3 sigmas
    error=3*sqrt(var2/(Nexp*1.0));
    
    //Sacamos el error estadistico por pantalla
    printf("%le \t %le \n",var2,error);
